...
//   vec2
//   point=scale*vec2(dot(angle.yx,vec2(uv.x,-uv.y)),dot(angle,uv));return(sin(point.x)*sin(point.y))*4.0;
//   }

void main() {
  // const in vec4 fragCoord,const in vec2 uv,out vec4 fragColor
  // vec4 solidRed = vec4(1.0-uv.x,1.0-uv.y,1.0-uv.y,1);//This is actually black
  // right now
